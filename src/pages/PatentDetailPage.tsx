...
export const PatentDetailPage: React.FC = () => {
  ...
  const { getPatentBySlug, isLoading } = usePublicContent();
  const patent = slug ? getPatentBySlug(slug) : undefined;
  ...
  if (isLoading && !patent) {
    return (
      <div className="pt-32 pb-24 max-w-4xl mx-auto px-4 sm:px-6 text-center flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-xs font-mono text-stone-500 uppercase tracking-widest">Loading Patent...</p>
      </div>
    );
  }
  ...
  if (!patent) {
    return (
      <div className="pt-32 pb-24 max-w-4xl mx-auto px-4 sm:px-6 text-center space-y-6">
        ...
        <p className="text-sm text-ink-600 font-sans">The requested patent disclosure could not be located in the archive.</p>
        <Link
          to="/patents"

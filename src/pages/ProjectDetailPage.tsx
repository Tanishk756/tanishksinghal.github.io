...
import { ArrowLeft, ArrowUpRight, CheckCircle2, AlertCircle, LoaderCircle } from 'lucide-react';

export const ProjectDetailPage: React.FC = () => {
  const { slug } = useParams<{ slug: string }>();
  const { getProjectBySlug, isLoading } = usePublicContent();
  ...
  if (isLoading && !project) {
    return (
      <div className="pt-32 pb-24 max-w-4xl mx-auto px-4 sm:px-6 text-center flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-xs font-mono text-stone-500 uppercase tracking-widest">Loading Case Study...</p>
      </div>
    );
  }
  ...
  return (
    <div className="pt-24 pb-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
      ...
      <div className="space-y-6 pb-8 border-b border-paper-400">
        ...
        {/* Metadata Strip */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 p-6 rounded-2xl bg-white border border-paper-400 shadow-xs text-xs font-sans">
          <div>
          ...
          <div>
            <span className="text-[10px] font-mono uppercase text-stone-500 block mb-1">CATEGORY</span>
            <span className="font-semibold text-ink-900 uppercase">{project.category}</span>
          </div>
        </div>
          ...
          {project.results?.metrics && project.results.metrics.length > 0 ? (
            <div className="grid grid-cols-2 gap-4 pt-2">
              {project.results.metrics.map((m, mIdx) => (
                <div key={mIdx} className="p-4 rounded-xl bg-paper-100 border border-paper-300">
                  ...
                  <span className="text-[11px] font-mono text-stone-500">{m.label} {m.unit ? `(${m.unit})` : ''}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="p-6 rounded-2xl bg-paper-100 border border-paper-300 space-y-2 text-xs font-sans text-ink-700">
              <p className="leading-relaxed">
                Hardware validation logs and quantified runtime benchmarks are documented with physical test trials.
              </p>
            </div>
          )}

...
import { Github, Mail, Linkedin, LoaderCircle } from 'lucide-react';

export const AboutPage: React.FC = () => {
  const { profile, organizations, education, isLoading, error } = usePublicContent();
  ...
  if (isLoading && !profile) {
    return (
      <div className="pt-32 pb-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-xs font-mono text-stone-500 uppercase tracking-widest">Loading Profile...</p>
      </div>
    );
  }
  ...
  if (error && !profile) {
    return (
      <div className="pt-32 pb-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
        <h1 className="text-2xl font-display font-bold text-ink-900">About</h1>
  ...
  const profileData = profile || {
    ...
    email: '',
    avatarUrl: '',
    socials: { github: '', linkedin: '', googleScholar: '', researchGate: '' },
    keyStats: [],
    source: '',
      ...
      <section className="p-8 rounded-3xl bg-white border border-paper-400 shadow-editorial flex flex-wrap items-center justify-between gap-6">
        <div>
          ...
          <p className="text-xs text-stone-500 font-mono">
            {profileData.fullName} {profileData.email ? `· ${profileData.email}` : ''}
          </p>
        </div>
